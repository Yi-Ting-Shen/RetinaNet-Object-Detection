...
def log_average_miss_rate(precision, fp_cumsum, num_images):
    """
    計算log-average miss rate:
    透過在 log-space 中的9個均勻分佈的 FPPI 點進行平均
    參考文獻:
...
# 計算給定recall和precision數組的AP
def voc_ap(rec, prec):
    rec.insert(0, 0.0) # 在列表的開頭插入0.0
    rec.append(1.0)    # 在列表的末尾插入1.0
    mrec = rec[:]
    prec.insert(0, 0.0) # 在列表的開頭插入0.0
    prec.append(0.0)    # 在列表的末尾插入0.0
    mpre = prec[:]
